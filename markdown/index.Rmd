...
# Read in district aggregates
d1_aggregates_nash <- st_read("~/GitHub/Nashville_IMV/data/metro/Subareas/Subareas/Subareas.shp") %>%
  st_as_sf(crs = 4326)
...
nash_venue_data <- read.csv("~/GitHub/Nashville_IMV/data/venue_tables/venue_table_detailed_7_3.csv")
...
base_map2 <-get_stamenmap(bbox = unname(st_bbox(ll(st_buffer(st_centroid(d1_aggregates_nash),10000)))),
                    force = TRUE, maptype = "toner-lite", zoom = 12)

base_map3 <-get_stamenmap(bbox = unname(st_bbox(ll(st_buffer(st_centroid(d1_aggregates_nash),10000)))),
