...
# Read in district aggregates
d1_aggregates_nash <- st_read("~/GitHub/Nashville_IMV/data/venue_tables/to_map/d1_aggregate_v2.geojson") %>%
  st_as_sf(crs = 4326) %>%
...
nash_venue_data <- st_read("~/GitHub/Nashville_IMV/data/venue_tables/to_map/venues_v2.geojson") %>%
...
Clean the data to add IMV and Music Space indicators

```{r}
 nash_venue_data <- nash_venue_data %>% 
...
- IMVs do not seem to be more community-focused on average
- About 2/3rds of music spaces in Nashville are "Occasional" music spaces - with programming focused on other types of business and/or infrequent events
...
This is very interesting - what are the non-indie venues that are more local drive?
...
- Venue zoning districts (bar plot) - WE STILL NEED THE ZONING

- Music spaces -  total appraised per sqft (histogram, table with summary stats)
...
Box and whisker plot for 1-4 with sq footage on the Y
...
fishnet_parcels <- st_join(fishnet_parcels, 
  ...
  group_by(uniqueID) %>% 
  summarise(medianAssessment = median(TotlAppr/Area_sqft, na.rm = TRUE)) %>%
  #mutate(n = ifelse(n == 1, 0, n)) %>%
  #rename(n_venues = n) %>%
  as.data.frame() %>%
  select(-geometry) %>%
