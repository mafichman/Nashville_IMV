...
- Write a methodology and data sources sections - 2 and 3 (Sofia - started)
- Annotate maps and charts, to contain proper titles, axis titles, and styling (Sofia)
- Add section number references to takeaways, refine risk indicators. (Michael)
...
ggplot()+
  ...
  geom_sf(data = fishnet %>%
          ...
          color = "transparent",
          alpha = 0.8)+
  scale_fill_gradient2(low=beige,high=purple,midpoint=18,mid=pink,limits=c(1,max(fishnet$n_venues)), breaks=c(1, max(fishnet$n_venues)), labels=c("1",max(fishnet$n_venues)),name="# Venues in 1/2-mile")+
  labs(
...
nash_venue_data %>%
  ...
  kable(
    col.names = c("Independence Status","Gross Capacity", "Median Capacity")
  ) %>% 
kable_styling(bootstrap_options=c('striped','hover',
  'condensed','responsive'),position='center',
   font_size = 18,full_width=F)
...
d1_aggregates %>%
  ...
  kable(
    col.names = c("City","Population","Number of Music Spaces","Area (sq m)","Number of Venues per 10k People","Venues Density (/sq m)")
  ) %>% 
kable_styling(bootstrap_options=c('striped','hover',
  'condensed','responsive'),position='center',
   font_size = 18,full_width=F) %>% 
...
nash_venue_data %>%
  ...
  kable(
    col.names = c("Independence Status",'Average "Score" on our Independent Programming Metric','Median "Score" on our Independent Programming Metric')
  ) %>% 
kable_styling(bootstrap_options=c('striped','hover',
  'condensed','responsive'),position='center',
   font_size = 18,full_width=F)
...
ggplot()+
  ...
  scale_fill_gradient2(low=beige,high=indigo,midpoint=c(20),mid=c(pink),limits=c(0,max(df$num_music_smaces)),breaks=c(0,5,20,max(df$num_music_smaces)), labels=c("0","5","20",max(df$num_music_smaces)),name="")+
...
nash_venue_data %>%
  ...
  as.data.frame() %>%
  group_by(IMV) %>%
  summarize(mean_dist = round(mean(centroid_dist) / 5280),
            median = round(median(centroid_dist) / 5280)) %>% 
  kable(
    col.names = c("Independence Status",'Average Distance from Center',"Median Distance from Center")
  ) %>% 
kable_styling(bootstrap_options=c('striped','hover',
  'condensed','responsive'),position='center',
   font_size = 18,full_width=F)
...
nash_venue_data %>%
  ...
  title = "Distance from Nashville's Music Center",
    subtitle = 'Comparing across independence status of venues',
    x="Distance from Music Center (miles)",
    y='Density of Venues',
...
SOFIA - can you look up the top 10 of these and see what they actually mean in the code?
...
ggplot()+
  geom_sf(data = fishnet_parcels %>%
          ...
          color = "transparent",
          alpha = 0.6)+
 scale_fill_gradient2(low=beige,high=indigo,midpoint=c(600),mid=c(pink),limits=c(0,max(na.omit(fishnet_parcels$medianAppraisal))),breaks=c(200,400,600,800), labels=c("200","400","600","800"),name="Price per Sq Foot")+
  geom_sf(data = d1_aggregates_nash %>%
