...
- Write top takeaways (Michael) - DONE PENDING NEW DATA - NEED TO ADD SECTION NUMBER REFERENCES
- Fix some venue data points - before re-running with licenses (Michael):
...
```
### 4.7.2. Venue Density by City

THERE IS SOMETHING WRONG WITH THE TOTAL POPULATION NUMBER HERE FOR NASHVILLE

Sydney's population represents only 2 of the 33 Local Government Areas in the metro region - City of Sydney and Inner West Council.
...
d1_aggregates %>%
  ...
  rbind(., d1_aggregates_nash %>%
          st_transform(2274) %>%
          mutate(city = "Nashville") %>%
          mutate(area_sqkm = st_area(.)* 0.00000000929) %>%
          as.data.frame() %>%
