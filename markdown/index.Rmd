...
- Analyze recent sales values and impart risk scores (Michael) - IN PROGREDSS
...
nash_venue_data %>%
  ...
  mutate(programming_sum = interdisciplinarity_ord + experimentation_ord +
           community_focus_ord + event_promotion_ord) %>%
  as.data.frame() %>%
  select(value_sqft,  name, address_admin, n_sales)%>%
  gather(-name, -address_admin, key = "variable", value = "value") %>%
  group_by(variable) %>%
  mutate(quantile = quantile(value, c(0.25, 0.5, 0.75), na.rm = TRUE)) %>%
  ungroup() %>%
  kable() %>%
  kable_styling()
