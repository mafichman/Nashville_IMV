...
283 venues, goal is to have all 238 assigned an APN value
```{r}
...
viewtable = left_join(work_table, work_join_table, by = c("APN_Address"="PropAddr")) %>% filter(is.na(APN)) %>% unique()
...
COPY of above chunk for updated version
```{r}
## loading manually populated venue database (last updated 5/11/2023)
pop_venue_db = read.csv("C:/Users/sofia/Documents/UPenn/Nashville/venues4_16.csv", header=TRUE)

## loading the manually matched database of venues <--> parcels that I made (last updated 6/7/2023) 
manual_match_table = read.csv("C:/Users/sofia/Documents/UPenn/Nashville/manual_APNs.csv", header=TRUE)

## select only the two address column to match and APN column from tax parcel data to have a simpler table to work with
work_join_table = select(parcels, PropAddr,APN)

## join by address to tax parcel data
APN_join = inner_join(manual_match_table, work_join_table, by = c("APN_Address"="PropAddr")) %>% unique()
...
## creating simple and detailed tables

## simple table (use for all administrative and venue data joins)
venue_admin_key_table = select(APN_join, c(Venue_name, APN_Address, APN))
...
venue_coordinates = APN_join %>% select(c(Venue_name,geometry))
...
## detailed table (all venue and parcel information)
detailed_table = left_join(venue_data_done,parcels,by= c("Admin_Address"="PropAddr","APN")) %>% unique()

## save the important tables to local files

## save simple table
#st_write(venue_admin_key_table, "C:/Users/sofia/Documents/UPenn/Nashville/final_tables/simple_table.csv")

## save venue table
#st_write(venue_data_done, "C:/Users/sofia/Documents/UPenn/Nashville/final_tables/venue_table.csv")

## save detailed table
#st_write(detailed_table, "C:/Users/sofia/Documents/UPenn/Nashville/final_tables/detailed_table.csv")
