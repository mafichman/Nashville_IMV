...
venues = read.csv("~/Github/Nashville_IMV/data/venue_tables/New_8_9/venue_8_9_populated.csv") %>% st_as_sf(coords = c("x","y"), remove = FALSE,crs= 4326, na.fail = FALSE)
...
venues = venues %>% mutate(
  independent_booking_and_ownership = ifelse(independent_booking_ord==1&ownership_structure_ord==1,1,0)
)

plan_dist_venues = st_join(plan_dist, select(venues,capacity,ownership_structure_ord,independent_booking_ord,independent_booking_and_ownership,independent_booking_and_ownership,interdisciplinarity_ord,event_promotion_ord,community_focus_ord,creativity_ord,experimentation_ord)) %>% 
